...
static void print_neighbour_header(void)
{
    ...
    print_neighbour_table_headers("cell", "north", "ne", "east", "se", "south", 
	"sw", "west", "nw", NULL);
}
...
{
    clause_t *cls;
    int clause_error = 0, literal_error = 0, literal_error_found = 0, ret = 0;
    int i;

    p_comment("clauses:");
    for (cls = test_clauses, i = 0; cls && i < sz; cls = cls->next, i++)
    {
	literal_t *lit, *expected_literals = expected_clauses[i].literals;
	...
	for (lit = cls->literals, j = 0; lit && j < expected_clauses[i].dim; 
	    lit = lit->next, j++)
	{
	    int error_print_literal = 0;

	    if (!literal_error_found && (lit->id != expected_literals[j].id || 
		lit->tv != expected_literals[j].tv))
	    {
		literal_error = 1;
		error_print_literal = 1;
	    }
	    print_literals(error_print_literal, &expected_literals[j], lit);
	}

	if (!literal_error_found && (j < expected_clauses[i].dim || lit))
	    literal_error += 2;
...
static int assert_table(table_t *expected_table)
{
	...
	{
	    if (!is_error && 
		(expected_table->t[i][j].id != test_table->t[i][j].id || 
		expected_table->t[i][j].tv != test_table->t[i][j].tv))
	    {
		is_error = 1;
		print_error = 1;
	    }
...
static int assert_neighbours(coordinate_euclid_t **table, int hight, 
    int length, coordinate_euclid_t **expected, coordinate_euclid_t **recieved)
{
	...
	print_neighbours(print_error, dim_x, i/hight, i%length, expected[i], 
	    recieved[i]);
    }
...
static int test24(void)
{
	...
	test_sat_parser_should_succeed("  (  a   or  -b  )   and   (  -c )  ");
}
...
{
    return test_sat_parser(NULL, test_variable_new, test26_success, parse_fail, 
...
{
    return test_sat_parser(test_clause_new, test_literal_new, test27_success, 
	parse_fail, "(a or b) and (-b or c or d) and (-b or -d) and (-c)");
}
...
static int test29(void)
{
	...
	"(a or b or b) and (-b or c or d) and (-b or -d or b) and (-c or -c)");
}
...
static int test31(void)
{
	...
	"(a or b or c or d) and (-c or a or -d or b) and (-b or -a or c or d) "
	"and (-d or -c or -b or -a) and (-d or c or a or b) and "
	"(d or -b or a)");
}
...
static int test32(void)
{
	...
	for (j = 0; j < 3 && l < 9; j++, l++)
	{
	    ...
	    {
		coordinate_moore_neighbour(3, 3, &co_table[i][j], dir, 
		    &recieved[l][dir]);
	    }
	}
    }
