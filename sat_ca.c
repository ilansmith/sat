...
static void loop_fail_set(void *o)
{
    ...
    for (i = 0; i < loop_len && loop->ca_list[i]->flag != FL_ERASE_LOOP; i++);
...
static ca_t **ca_space_alloc(int hight, int length)
{
	...
	for (j = 0; j < length; j++)
	{
		...
		cell.n = i;
		cell.m = j;
		coordinate_moore_neighbour(hight, length, &cell, dir, 
		    &neighbour);
		sp[i][j].neighbours[dir] = &sp[neighbour.n][neighbour.m];
	    }
	}
    }
...
static void mon_spread_do(void *o)
{
    ...
    ca_t *nei_left = ca->neighbours[wind_dir_offset(cadir2dir(ca->dir), -2)];
...
static void mon_spread_scan(void *o)
{
    ...
    if (!is_bound(ca) || 
	(!is_bound(ca->neighbours[wind_dir_offset(cadir2dir(ca->dir), -2)]) && 
	!is_bound(ca->neighbours[cadir2dir(ca->dir)])))
    {
	/* if the cell is not bound or neither the cell ahead of it and the 
	 * cell to its left are bound initiate rescanning */
...
static ca_monitor_state_t mon_scan(monitor_t *mon, int id, ca_code_t code)
{
	...
	if (!table->t[mon->pos_tbl[i]][clause].id && 
	    table->t[mon->pos_tbl[i]][clause].tv == SAT_TV_TAUTOLOGY)
	{
	    return MN_FAIL;
	}
	...
	if (table->t[mon->pos_tbl[i]][clause].tv == SAT_TV_PARADOX || 
	    table->t[mon->pos_tbl[i]][clause].tv != code2code(truth_values, 
	    code))
	{
...
static int is_arm_extrusion_failure(ca_t *ca)
{
	...
	ca->neighbours[wind_dir_offset(cadir2dir(X), -2)]->code == CD_FLOW);
...
static void flags_scan(ca_t *ca)
{
    ...
    /* if the replication arm is closing on itself set the flag to mutate a new
     * one bit */
    ...
    /* arm extrusion failure checking. a failed attempt at new arm extrusion 
     * will result in the FL_BRANCH_SEQ flag being set in the corner. this 
     * allows further attempts at the other directions later */
    ...
    /* change FL_GEN_ZERO to FL_GEN_ONE after seeing a CD_TURN_LEFT in order 
     * to mutate a new one bit */
    ...
    /* reset FL_GEN_ONE/FL_GEN_ZERO to either FL_QUIESCENT or FL_BRANCH_SEQ */
	...
	if (ca->code == CD_UNEXPLORED_0 || ca->code == CD_UNEXPLORED_1)
	{
	    flag_set(ca, FL_BRANCH_SEQ);
	    return;
	}
...
static int is_gen_arm_extention(ca_t *ca)
{
	...
	ca->flag == FL_BRANCH_SEQ && pointing_neighbour(ca)->code == CD_FLOW;
}
...
static int is_arm_ext_start_corner(ca_t *ca)
{
	...
	pointing_neighbour(pointing_neighbour(ca))->code == CD_ARM_EXT_END);
...
static void bound_rules_scan(ca_t *ca)
{
    ...
    {
	ca_direction_t dir = dir2cadir(wind_dir_offset(cadir2dir(ca->dir), 2));
	ca_code_t code = 
	    ca->neighbours[wind_dir_offset(cadir2dir(ca->dir), -2)]->code;
...
static ca_direction_t is_neighbour_grow(ca_t *ca)
{
	...
	cadir2dir(ca->neighbours[wind_dir_offset(cadir2dir(X), 1)]->dir) != 
...
static int is_arm_extend(ca_t *ca)
{
    ...
    ROTATE(dir, ca, ca->neighbours[cadir2dir(X)]->code == CD_ARM_EXT_START && 
	ca->neighbours[cadir2dir(X)]->dir == dir_opposite(X) && 
	ca->neighbours[cadir2dir(dir_opposite(X))]->dir == DR_QUIESCENT &&
	(ca->neighbours[cadir2dir(X)]->flag == FL_QUIESCENT || 
	ca->neighbours[cadir2dir(X)]->flag == FL_MONITOR_ACTIVE));
...
static int is_turn_left(ca_t *ca)
{
	...
	wind_dir_offset(cadir2dir(X), -2) && 
	ca->neighbours[wind_dir_offset(cadir2dir(X), -1)]->dir == 
	DR_QUIESCENT && (ca->neighbours[cadir2dir(X)]->flag == FL_QUIESCENT || 
	ca->neighbours[cadir2dir(X)]->flag == FL_MONITOR_ACTIVE));
...
static void unbound_rules_scan(ca_t *ca)
{
	...
	{
	    code_set(ca, CD_FLOW);
	    col_set(ca, ca->neighbours[cadir2dir(dir_opposite(dir))]->col);
	}
	return;
    }
...
static ca_t *loop_assignment_get_next(void)
{
    ...
    for (ca = loop_queue_success->ca_list; (*ca)->code != CD_TURN_LEFT; ca++);
...
static int assignments_set(void)
{
    ...
    for (queue = loop_queue_success, i = 0; queue; queue = queue->next, i++);
	...
	{
	    table_assignment_insert(table, i, id, code2code(truth_values, 
		ca->code));
...
static void sat_epilogue(void *o)
{
    ...
    event_add(assignments_set() ? sat_error_handler : sat_success_handler, o);
}
...
static void sp_initial_configuration(void *o)
{
	...
	s->sp[s->y0][x_offset].code = i < s->loop_dim ? CD_GROW : CD_FLOW;
	...
	s->sp[y_offset][s->x0].dir = y_offset == s->y0 - s->loop_dim + 1 ? 
	    DR_LEFT : DR_DOWN;
	    s->sp[y_offset][s->x0].col = CL_RED;
	...
	s->sp[y_offset][x_offset].dir = i == s->loop_dim - 1 ? DR_UP : DR_LEFT;
    ...
    sat_print_t *printer)
{
	...
	printer->is_bright = (sat_print_code[idx].is_bright + is_pulse) % 2;
    }
}
