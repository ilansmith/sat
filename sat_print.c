...
static void sat_print(void *o)
{
	...
	for (j = 0; j < dim; j++)
	{
	    ...
	    sprintf(colour, FMT_COLOUR, printer.is_bright, printer.colour, 
		COL_BCK_BLACK);
	    sprintf(field, "%s%c%s", colour, printer.representation, COL_CLEAR);
	    fprintf(stdout, "%s", field);
	}
	print_border_horizontal();
	fprintf(stdout, "\n");
...
void sat_print_results(table_t *table, variable_t *variables)
{
	...
	{
	    fprintf(stdout, "%s=%-7s", 
		literal_id2name(variables, table->assignments[i][j].id), 
		table->assignments[i][j].tv ? "TRUE" : "FALSE");
	}
	fprintf(stdout, "\n");
    }
    fflush(stdout);
